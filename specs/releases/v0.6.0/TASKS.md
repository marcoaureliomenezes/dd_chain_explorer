...
- [x] **T-I.8** — **OPERATOR-ONLY.** Apply `services/prd/00_bootstrap` with operator credentials — the sole documented exception to ADR-6's law, and the only stack CI may never apply (O-2).
  - Evidence: done 2026-08-31 (operator-authorized in-session apply, iam user): pre-apply plan matched the T-I.7-approved 3 add / 4 change / 0 destroy exactly; apply summary "3 added, 4 changed, 0 destroyed"; post-apply plan "No changes"; list-roles shows all 5 gha roles; simulate-principal-policy on gha-artifacts-publish: s3:PutObject on artifacts key -> allowed, state-bucket s3:GetObject / lambda:UpdateFunctionCode / iam:PassRole -> implicitDeny. **Trust flipped: legacy dd_chain_explorer workflows can no longer assume any role.** Applied at infra commit bbb9f9b (fix chain f38bd86..bbb9f9b included).
...
- [x] **T-I.11** — Inventory and declare the Databricks workspace infrastructure (I7, ADR-5, K4): list every existing Unity Catalog **storage credential**, **external location** and **catalog** (`databricks storage-credentials list`, `external-locations list`, `catalogs list`), then author a Terraform stack declaring exactly those objects — no more, no fewer. DLT pipelines, workflows, jobs and dashboards are **not** declared; they stay DABs.
  - Evidence: done c52511d: inventory 4/4/6 observed -> 10 declared (3 SC + 3 EL + 4 catalogs; system-managed/SYSTEM_CATALOG objects unrepresentable, excluded with reasons in INVENTORY.md; residual de-lakehouse-credential + 2 Marketplace catalogs declared faithfully); stack services/databricks (provider 1.124.0, new state key databricks/unity-catalog); IMPORT-COMMANDS.md ready for T-I.12; fmt/validate/pytest 99/ruff/mypy clean; kept out of stack_map until creds+state exist (README-documented).
...
- [x] **T-I.14** — Re-author the six infrastructure workflows (I8) — plan-on-PR, deploy, destroy, destroy-all, drift detection, Scorecard — keeping the v0.5.0 posture (SHA-pinned actions under the allowlist, `persist-credentials: false`, runner hardening, per-environment `concurrency`, `-lock=false` on the read-only plan path, `actionlint` + `zizmor` + `terraform fmt/validate` gates, the OIDC preflight on every role-assuming job). **No application-deploy workflow travels.** Fold in the remaining two security LOW residuals: the raw interpolation in the deploy workflow becomes an environment binding, and the preflight masks the role ARN.
  - Evidence: done e1da420+39bb590 (pushed): 6 workflows re-authored, prd-iam dropped, app lane removed, handler resolvers wired, 2 security LOWs folded (env-binding no tag job; ARN mascarado no preflight); gates yaml/actionlint+shellcheck/zizmor-high 0/ruff/mypy/pytest 99 passed 0 skips (3 re-enabled).
...
- [-] **T-X.5** — Implement the **producer** half of the artifact seam (X3, ADR-3, K3): CI builds the layer (`pip install --require-hashes -r requirements.lock -t build/` for third-party deps **plus** `pip install ./utils -t build/ --no-deps` — the path install is what closes dependency confusion) and both handler zips, uploads each to its content-addressed key under the bucket and prefix shape pinned by `T-L.3`, assuming **only** `…-gha-artifacts-publish`, and emits each `<sha256>` as a run output the infrastructure repo's resolver can name.
  - Progress: authored+pushed c391c7e (publish-artifacts.yml) + T-X.6 authored 7e45066 (ci.yml) — all local gates green (actionlint/zizmor 0, pytest 62+1, handler-zip reproducibility proved); live acceptance blocked on T-I.8/T-X.2.
