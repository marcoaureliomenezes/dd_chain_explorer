...
> **Amended:** 2026-09-02 — ADR-10 (two environments: `hml` retired, `prod` Databricks = the official account); every `hml` enumeration below rewritten under it.
...
| I2 | **Terraform moved as content, state untouched.** The surviving root stacks (`dev/01_peripherals`, `dev/02_lambda`, `prd/00_bootstrap`, `prd/01_tf_state`, `prd/04_peripherals`, `prd/06_lambda`; `hml/04_peripherals` travelled and is then destroyed and removed under ADR-10) and the four modules (`cloudwatch_logs`, `dynamodb`, `lambda`, `s3`) move with **identical backend keys and identical resource addresses**, so every stack plans `0 to add, 0 to change, 0 to destroy` from the new repo. Committed `.terraform.lock.hcl` files travel with their stacks. **The empty `prd/03_iam` stack does not travel** — it declares no resource; its state key is removed instead of copied (v0.5.0 intake residual, folded here because a migration is the one moment the decision is free). |
...
| I6 | **OIDC repointed** (ADR-4). `prd/00_bootstrap` gains a `github_repo` variable; the deploy roles `…-gha-deploy-{dev,prd}` and `…-gha-readonly-plan` re-trust (`…-gha-deploy-hml` is removed in the ADR-10 bootstrap delta, after the `hml` destroy) `repo:<owner>/dd-chain-infrastructure:…` with their existing `sub` pinning (`:environment:<env>` per deploy role, `pull_request` + `refs/heads/{develop,main}` for the read-only role). A **new narrow role `…-gha-artifacts-publish`** is added, trusted to `repo:<owner>/dd-chain-explorer:environment:{dev,production}`, granting **only** `s3:PutObject`/`s3:PutObjectTagging`/`s3:AbortMultipartUpload` on `arn:…:<artifacts-bucket>/*` plus `s3:ListBucket` on the bucket — no other action, no other resource, and the project permissions boundary attached like every other project role. `00_bootstrap` remains **operator-applied, never CI-applied**. |
...
| I9 | **Variables and the credential surface re-established.** `scripts/ci/publish_oidc_vars.sh` runs against the repointed bootstrap outputs and publishes `AWS_DEPLOY_ROLE_{DEV,PRD,READONLY}` in this repository (`_HML` deleted with the environment, ADR-10); the fail-fast preflight guards every role-assuming job. No static key, ever. |
...
| X4 | **Explorer CI.** Quality gate (`ruff format --check`, `ruff check`, `mypy`, `pytest -p no:cacheprovider`, `pip-audit -r` over the lock), the layer/handler build + publish job, `databricks bundle validate -t dev` per bundle on every PR/push and `-t prod` on pushes to `main` (bound to `production`), and the DABs deploy lane behind the informed environment gate. `actionlint` + `zizmor` clean, every action SHA-pinned, `persist-credentials: false`, runner hardening — the v0.5.0 posture, reproduced. This repository has **no** Terraform workflow and **no** deploy role beyond `artifacts-publish`. |
| X5 | **Bundles validated and deployed from the new repo.** All 7 bundles validate in `dev` (Free Edition) and `prod` (the official account, ADR-10); `dev` deploys from CI, `prod` deploys only through the `production` gate. Deployed `dev` workspace state equals the new repo, proven by notebook export diff. Bundle content changes are limited to the target set (`hml` removed, `prod` real: `mode: production`, deployed as the production service principal, no hard-coded application id). |
...
Databricks `dev` and `prod` targets · objects under the artifacts bucket's layer and
...
| V3 | **Unity Catalog proof:** the imported storage credentials and external locations validate; the `hml` buckets no longer exist (`head-bucket` → 404, ADR-10); every bundle validates in `dev` and `prod` and deployed `dev` state equals the new explorer repo. |
...
environments `dev` / `production` with their reviewers and deployment-branch
...
| AC-2 | I | `ls services/prd services/dev services/modules` | the 6 surviving root stacks and 4 modules are present; `prd/03_iam` and `services/hml` are **absent**; a `.terraform.lock.hcl` exists for every root stack |
...
| AC-13 | X | `make dabs_validate_all TARGET=dev` and `TARGET=prod` (each against its own workspace credentials); notebook export diff per deployed `dev` pipeline | exit 0 for all 7 bundles in both targets; `diff` exit 0 for every deployed pipeline against its file in the **new** repo |
...
| `services/dev/{01_peripherals,02_lambda}` | same paths | identical backend keys |
| `services/hml/04_peripherals` | travelled, then **destroyed via the CI destroy lane and removed** (ADR-10) | `head-bucket` 404 on both hml buckets; state key gone; no `services/hml/` in the tree |
...
- **ADR-10 — Two environments (operator ruling 2026-09-02, amends this SPEC in flight).**
  *"There is no hml any more. dev = the Databricks Free Edition workspace; prod = my
  official Databricks account."* Consequences: the `hml` AWS stack is destroyed through
  the CI destroy lane and its code removed; `…-gha-deploy-hml` leaves the bootstrap;
  `…-gha-artifacts-publish` trusts `environment:{dev,production}`; every bundle has
  exactly the targets `dev` and `prod`, `prod` deployed only through the `production`
  gate as the production workspace's own service principal; both repositories carry
  the GitHub environments `dev` and `production` only. Supersedes memory ADR-002
  ("one workspace, catalogs as environments") — rewritten at CLOSURE (§8).
