  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${data.terraform_remote_state.s3.outputs.databricks_bucket_arn}/exports/*"
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_lambda_permission" "s3_invoke" {
  ...
  source_arn    = data.terraform_remote_state.s3.outputs.databricks_bucket_arn
}

resource "aws_s3_bucket_notification" "gold_export" {
  bucket = data.terraform_remote_state.s3.outputs.databricks_bucket_name
