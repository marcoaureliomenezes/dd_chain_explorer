...
dm_test_api_keys.py
-------------------
Busca todas as API Keys do AWS SSM Parameter Store (por hierarquia) e testa se
estão funcionando.

Hierarquia SSM esperada
  /web3-api-keys/alchemy/api-key-N   → Web3 HTTPProvider (Alchemy)
  /web3-api-keys/infura/api-key-N    → Web3 HTTPProvider (Infura)
  /etherscan-api-keys/api-key-N      → Etherscan REST API
...
Uso rápido:
    python -m chain_utilities.dm_test_api_keys                  # mainnet, 5 reqs por chave
    python -m chain_utilities.dm_test_api_keys --num-reqs 20    # 20 reqs por chave
    python -m chain_utilities.dm_test_api_keys --network goerli

Uso como container (sem credenciais locais — assume IAM role/ECS task role):
    docker run --rm \\
      -e AWS_DEFAULT_REGION=sa-east-1 \\
      <image> python -m chain_utilities.dm_test_api_keys
"""
...
# SSM paths → vendor (order determines iteration order in the report)
...
class APIKeysTester:
    ...
    def _fetch_keys_by_path(self) -> list[tuple[str, str, str]]:
        """
        Busca todos os parâmetros SSM usando get_parameters_by_path para
        cada prefixo hierárquico conhecido.  Retorna lista de (name, value, vendor).
        """
        result: list[tuple[str, str, str]] = []
        for path, vendor in SSM_PATHS.items():
            logger.info(f"Buscando parâmetros em '{path}' [{vendor}]…")
            params = self._ssm.get_parameters_by_path(path)
            if not params:
                logger.warning(f"  Nenhum parâmetro encontrado em '{path}'.")
                continue
            for name, value in sorted(params.items()):
                result.append((name, value, vendor))
            logger.info(f"  {len(params)} chave(s) encontrada(s) em '{path}'.")
        logger.info(f"Total de API Keys encontradas: {len(result)}")
        return result

    def run(self) -> list[KeyTestResult]:
        keys = self._fetch_keys_by_path()

        for name, value, vendor in keys:
