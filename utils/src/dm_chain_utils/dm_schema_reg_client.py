...
def _set_compatibility(subject: str, mode: str = "BACKWARD") -> None:
    """Set per-subject compatibility mode on Confluent Schema Registry."""
    url = f"{SCHEMA_REGISTRY_URL}/config/{subject}"
    r = requests.put(url, json={"compatibility": mode}, timeout=5)
    r.raise_for_status()
    logger.info(f"[SchemaRegistry] Compatibility for '{subject}' set to {mode}.")


def _confluent_register_if_missing(subject: str, schema_path: str) -> None:
    """Register schema on Confluent SR if it does not exist yet, then enforce BACKWARD compatibility."""
...
def _confluent_get_schema(subject: str, schema_path: str | None) -> str:
    """Get schema definition string from Confluent Schema Registry."""
    if schema_path is not None:
        _confluent_register_if_missing(subject, schema_path)
    else:
        logger.info(f"[SchemaRegistry] Fetching '{subject}' from Confluent SR (no auto-register).")
    url = f"{SCHEMA_REGISTRY_URL}/subjects/{subject}/versions/latest"
    r = requests.get(url, timeout=5)
    r.raise_for_status()
    schema_str = r.json()["schema"]
    logger.info(f"[SchemaRegistry] Schema '{subject}' fetched from {SCHEMA_REGISTRY_URL}.")
    return schema_str
