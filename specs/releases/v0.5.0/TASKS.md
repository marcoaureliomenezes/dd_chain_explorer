...
- [x] **T-A.5** — Deactivate the CI IAM user's 2025-vintage access key (A2).
  ...
  - Coordinator 2026-08-23: DONE — CI IAM user legacy access key (created 2025-05-05) set Inactive; `aws iam list-access-keys` → Inactive/Active(2026-06-15 operator key).
...
- [x] **T-A.7** — CI safety batch: per-environment `concurrency`, PR trigger on both long-lived branches, the three test suites + `pip-audit` + the from-source layer build wired into `plan_on_pr`, `destroy_all` covering every live stack, Terraform and actionlint installer pinned (version + checksum), `auto-bump-version.yml` deleted, every `hml-apps` reference removed (A4, A5).
...
- [x] **T-B.1** — Force-unlock the two state locks stuck since April (B3). **First live task — O-2 blocks every plan/apply behind it.**
  ...
  - Coordinator 2026-08-23: DONE — 2 stale Apply lock rows (2026-04-22) deleted from the lock table after `gh run list --status in_progress` = 0; scan of rows with Info → 0. Evidence `.dadaia/tmp/software-engineer/20260823/live/lock-table-before.json`.
...
- [x] **T-B.7** — Disable the hourly PRD `contracts-ingestion` schedule **through Terraform**; keep the Lambda and the `job_export_gold → gold_to_dynamodb → DynamoDB` chain (B4).
  - Evidence: DEFERRED (operator-ratified): schedule already DISABLED live via the scheduler API; the Terraform path rides on T-B.14's artifacts store. Disposition in CLOSURE ## Dispositions; residual apply is an intake candidate.
...
- [x] **T-B.14** — Artifact store and layer inputs (B6, D15): create `dm-chain-explorer-artifacts` (versioned, private, block-public-access) in `prd/04_peripherals` — dev consumes it under a `dev/` prefix; rewire `prd/06_lambda` and `dev/02_lambda` to read the layer from `s3_bucket`/`s3_key`/`source_code_hash` supplied as `layer_s3_key`/`layer_sha256` **variables** (no `filebase64sha256` on a working-tree path) and to build the handler zips with `data "archive_file"` over `apps/lambda/<fn>/src`; declare and **import** the kept Lambda log groups with their retention.
  - Evidence: DEFERRED (operator-ratified): creating dm-chain-explorer-artifacts is a new productive PRD resource, excluded by the operator's scope. Code is ready (plan green with skip-with-warning); apply is an intake candidate carried with T-B.7.
...
- [x] **T-C.5** — Validate every surviving bundle in `dev` and `hml` before any deploy (O-6).
  - Owner: software-engineer · Write set: none (verification)
  - Deps: T-C.4 · AC-17 · Findings: DRIFT-18
  - Evidence: exit-0 `databricks bundle validate -t dev` and `-t hml`, captured per bundle name.

- [x] **T-C.6** — Deploy `dev` and `hml` so live == repo (incl. the never-deployed Fluent-Bit app-logs reader and the pre-R1 hml ethereum pipeline); remove the stale `.bundle/dd-chain-explorer` roots and the orphan dashboard (C5). **No prod deploy.**
...
- [x] **T-E.4** — **(CLOSURE)** Author the capture-deprecation ADR (supersession by `dd-chain-capture`, S3 as the sole boundary, parked-until-delivery posture, sunset criteria) and rewrite the ADR-005 lambda-union claim to the streaming-only reality (E2).
  ...
  - Done 2026-08-23 (product-engineer, CLOSURE phase; no shell — reservation flip not committed separately, it rides the closure commit): `specs/memory/architecture.md` §Referência now carries **ADR-007 "Capture is deprecated here and superseded by dd-chain-capture"** (decision, S3 raw bucket as the sole boundary, parked-until-delivery posture, the forbidden-reintroduction list, and three sunset criteria — first end-to-end delivery processed, field-name compatibility validated, `capture/ecr` state + KMS transferred); **ADR-005 rewritten** to "Transactions gold view is streaming-only" — the union claim is gone, the `batch_sem_decode` rank is recorded as having no producer while the hourly schedule is disabled and the `CONTRACT` entities are empty, the Lambda kept as dormant capability; **ADR-004 gained the corollary** "no bundle references another bundle's resource" (T-C.2/F-06 — cross-bundle `${resources.*.id}` does not resolve and a display-name lookup would couple bundles through `[dev]` prefixes; fan-out bundles do not exist).
...
- [x] **T-E.5** — **(CLOSURE)** Update the memory atoms to the post-release truth (E5, SPEC §8): `product/cicd-pipeline.md`, `product/aws-resources.md`, `architecture.md`, `product/medallion-pipelines.md`, `product/serving-layer.md`, `product/data-catalog.md` (only if the object inventory changed), `product/capture-layer.md`, `quality-assurance.md`, `tech-stack.md`, and `product/index.md` + `catalog.json` only if a rank changed or an atom was added/removed.
  ...
  - Done 2026-08-23: 8 atoms written to post-release truth — `architecture.md` (stacks/contracts/topology/deploy order + ADRs), `product/cicd-pipeline.md` (rewritten: OIDC live, quality gate, hardening, governance; gap list deleted), `product/aws-resources.md` (rewritten: stack↔state-key table, OIDC roles + boundary, hml lane, artifacts bucket declared-not-applied, 7 state keys, "Retirado do inventário"), `product/medallion-pipelines.md` (no DLT `schedule:`, in-bundle triggers, deployed==repo), `product/serving-layer.md` (parametrised dashboards, SPN `run_as`, alerts/Genie off the surface), `product/capture-layer.md` (parked posture → ADR-007, no residue), `quality-assurance.md` (143 tests / 4 suites / all in CI + PR gates), `tech-stack.md` (path-installed library, pinned providers + lock files, one version axis). **No change:** `product/data-catalog.md` (object inventory unchanged — no schema deployed), `product/index.md` + `catalog.json` (no rank change, no atom added or removed). `dadaia specs doctor` not run here — product-engineer has no shell; the coordinator runs it before archive.

- [x] **T-E.6** — **(CLOSURE)** Write `CLOSURE.md`: summary, tasks + commit shas, validations, size accounting, drifts, memory updates, `## Dispositions` with one terminal row per DRIFT-01..31 and per 2026-06-11 finding id (SPEC §7 is the source), test dispositions (T-D.2's map), record-only observations, artifact GC sweep, archive decision.
  ...
  - Done 2026-08-23: `specs/releases/v0.5.0/CLOSURE.md` written, **Status: Aprovado** — summary, 52-row task table (commit anchors `803238c`/`4d476ac`/`9ad2165`), validations table, size accounting recorded as **owed and unmeasured** (no shell — the `git diff --numstat` command is stated rather than guessed at), 5 drifts, memory updates (8 written + 3 explicit no-change), `## Dispositions` with 18 consumed backlog slugs, 2 bug rows, **DRIFT-01..31 one terminal row each** and **all 82 June lane ids** dispositioned by SPEC §7.2 group (fixed / superseded / deferred / rejected / record-only), test dispositions from T-D.2's verdict, 5 record-only observations, and an artifact GC sweep deferred to T-E.8 (its evidence is still read by the open re-audit). Archive decision: **MOVE, blocked** on the T-R.4 ship gate (6.1 live vs ≥7).

- [x] **T-E.7** — **(CLOSURE)** Compile the residuals into CLOSURE `## Intake candidates` for the PM's intake report — `terraform-single-stack-tree-per-env-tfvars`, `capture-ecr-state-and-kms-ownership-transfer`, `rest-api-public-endpoint`, `encryption-at-rest-posture-decision`, alert/Genie reinstatement. **No backlog entry is authored by this release.**
