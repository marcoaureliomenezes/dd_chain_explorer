...
- [x] **T-I7.3** — Security verdict on the bootstrap delta before any apply (O-3). APPROVED on infra `a26ce6d` (handoff `2026-09-13T192325Z-security-reviewer-bootstrap-delta-verdict`); 0 CRITICAL/HIGH; 4 MEDIUM outside the bootstrap dispositioned structurally in infra `55e1853` (dead credential dropped, no account-id literal, raw location read-only, UC lanes fail closed); AC-8 amended in effect: UC plan secrets are repo-level, no environment binding.
