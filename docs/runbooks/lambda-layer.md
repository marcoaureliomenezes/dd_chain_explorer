...
`services/prd/06_lambda` declares `layer_s3_key` / `layer_sha256` with **no
default** (T-B.14) — every plan or apply of that stack must be told which
built `dm-chain-utils` layer object to attach. Two different mechanisms
supply those values, for two different kinds of run:

| Run | Source of `layer_s3_key`/`layer_sha256` |
|---|---|
...
- A **scheduled drift check** (`drift_detection.yml`) that reported a
  false-positive `layer_s3_key`/`layer_sha256` diff on every single run,
