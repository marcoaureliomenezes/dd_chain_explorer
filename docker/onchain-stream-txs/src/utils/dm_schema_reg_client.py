"""
Schema Registry abstraction layer.

Always uses Confluent Schema Registry (via SCHEMA_REGISTRY_URL env var).
In production this points to the ECS-hosted Confluent SR reachable via Cloud Map DNS.
In development it points to a local Docker instance.

Usage:
    from utils.dm_schema_reg_client import get_schema

    schema_str = get_schema(
        schema_name="mined-block-event-schema",
        schema_path="schemas/1_mined_block_event_schema_avro.json",  # auto-registers if missing
    )
"""

import json
import logging
import os

import requests

logger = logging.getLogger(__name__)

SCHEMA_REGISTRY_URL = os.getenv("SCHEMA_REGISTRY_URL", "http://schema-registry:8081")


def _register_if_missing(subject: str, schema_path: str) -> None:
    """Register schema on Confluent SR if it does not exist yet."""
    check_url = f"{SCHEMA_REGISTRY_URL}/subjects/{subject}/versions/latest"
    r = requests.get(check_url, timeout=5)
    if r.status_code == 200:
        logger.info(f"[SchemaRegistry] Schema '{subject}' already registered.")
        return

    with open(schema_path) as f:
        schema_json = f.read()

    payload = {"schema": schema_json, "schemaType": "AVRO"}
    register_url = f"{SCHEMA_REGISTRY_URL}/subjects/{subject}/versions"
    r = requests.post(register_url, json=payload, timeout=5)
    r.raise_for_status()
    logger.info(f"[SchemaRegistry] Schema '{subject}' registered (id={r.json()['id']}).")


def get_schema(schema_name: str, schema_path: str | None = None) -> str:
    """
    Return the AVRO schema definition string for *schema_name*.

    Parameters
    ----------
    schema_name : str
        Name of the schema/subject (Confluent SR subject name).
    schema_path : str, optional
        Path to the local .json file used to auto-register the schema
        in Confluent SR if it does not exist yet.
    """
    if schema_path is not None:
        _register_if_missing(schema_name, schema_path)
    else:
        logger.info(f"[SchemaRegistry] Fetching '{schema_name}' from Confluent SR (no auto-register).")

    url = f"{SCHEMA_REGISTRY_URL}/subjects/{schema_name}/versions/latest"
    r = requests.get(url, timeout=5)
    r.raise_for_status()
    schema_str = r.json()["schema"]
    logger.info(f"[SchemaRegistry] Schema '{schema_name}' fetched from {SCHEMA_REGISTRY_URL}.")
    return schema_str
