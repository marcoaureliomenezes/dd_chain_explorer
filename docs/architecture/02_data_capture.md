...
1. **Streaming**: Pipeline de 5 jobs Python encadeados via AWS Kinesis/SQS/Firehose, executando em tempo real. Dados entregues no S3 como NDJSON: streams de blocos e decoded via Firehose Direct Put; stream de transactions-data via Kinesis → Firehose.
2. **Batch**: Jobs sob demanda para ingestão de contratos e limpeza de dados.

Toda a comunicação entre jobs utiliza serviços gerenciados AWS: **Kinesis Data Streams** (transactions-data), **Firehose Direct Put** (blocks-data, decoded-txs), **SQS** (coordenação inter-job) e **CloudWatch Logs** (logs de aplicação). Serialização é JSON nativo (sem Avro/Schema Registry).
...
flowchart LR
    ...
    K2["Firehose Direct Put<br/>mainnet-blocks-data"]
    SQS3["SQS<br/>block-txs-hash-id"]
    K4["Kinesis<br/>transactions-data"]
    K5["Firehose Direct Put<br/>mainnet-txs-decoded"]
    CW["CloudWatch Logs"]
...
| **Entrada SQS** | `mainnet-mined-blocks-events-{env}` |
| **Saída Firehose Direct Put** | `firehose-mainnet-blocks-data-{env}` (dados completos do bloco) |
...
| **Lógica** | Busca dados completos do bloco via RPC. Publica dados do bloco diretamente no Firehose (Direct Put) e faz fan-out de hashes de transações em SQS. Limita transações por bloco com `TXS_PER_BLOCK` (default 50). |
...
| Variável | Descrição | Exemplo |
|----------|-----------|--------|
| `FIREHOSE_STREAM_BLOCKS` | Stream Firehose Direct Put para dados do bloco | `firehose-mainnet-blocks-data-prd` |
...
| **Saída Firehose Direct Put** | `firehose-mainnet-transactions-decoded-{env}` |
...
| `FIREHOSE_STREAM_DECODED` | Stream Firehose Direct Put de saída | `firehose-mainnet-transactions-decoded-prd` |
| `CLOUDWATCH_LOG_GROUP` | Log group | `/apps/dm-chain-explorer-prd` |
...
| **Firehose Direct Put** | `firehose-mainnet-blocks-data-{env}` | JSON (18+ campos) | Job 3 | — | `raw/mainnet-blocks-data/` |
...
| **Firehose Direct Put** | `firehose-mainnet-transactions-decoded-{env}` | JSON (5 campos) | Job 5 (×3) | — | `raw/mainnet-transactions-decoded/` |

### 2.2 Configuração

**Kinesis Data Streams:** PROVISIONED 1 shard (`mainnet-transactions-data`)
**Firehose Direct Put:** 2 streams gerenciados sem Kinesis intermediário (`mainnet-blocks-data`, `mainnet-transactions-decoded`)
...
| `dm_firehose.py` | `FirehoseHandler` | Producer direto para Firehose (put_record, put_record_batch) |
