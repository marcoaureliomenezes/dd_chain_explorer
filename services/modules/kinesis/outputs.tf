...
output "firehose_direct_put_stream_names" {
  description = "Map of logical name → Firehose Direct Put delivery stream name"
  value       = { for k, v in aws_kinesis_firehose_delivery_stream.direct_put : k => v.name }
}

output "firehose_direct_put_stream_arns" {
  description = "Map of logical name → Firehose Direct Put delivery stream ARN"
  value       = { for k, v in aws_kinesis_firehose_delivery_stream.direct_put : k => v.arn }
}
