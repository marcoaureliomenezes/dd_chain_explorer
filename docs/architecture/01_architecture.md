...
1. **Captura on-chain** — Serviços Python containerizados que extraem dados da Ethereum via APIs Web3 (Alchemy/Infura) e publicam em Kinesis Data Streams ou Firehose Direct Put (dados) ou SQS (coordenação inter-job).
2. **Streaming & Ingestão** — Kinesis Firehose (Kinesis-source ou Direct Put) entrega NDJSON no S3; CloudWatch Logs + Firehose entrega logs de aplicação no S3.
...
└──────────────┬───────────────────────────┘
    ...
    ▼          ▼                         ▼
┌────────┐ ┌────────────────────────────────────┐ ┌──────────────────┐
│  SQS   │ │ Kinesis (txs-data, PROVISIONED 1sh) │ │ CloudWatch Logs  │
│(coord) │ │ Firehose Direct Put (blocks, decoded)│ │  (app logs)      │
└────────┘ └─────────────┬──────────────────────┘ └────────┬─────────┘
                         │                                  │
                         ▼                                  ▼
                   ┌──────────────────────────┐
...
Cinco jobs Python executam em containers Docker e formam uma pipeline encadeada de extração de dados on-chain. Jobs de coordenação (1→2, 3→4) comunicam via SQS. Jobs de dados publicam registros JSON como NDJSON no S3 via Firehose: Job 2 e Job 4 usam Kinesis → Firehose (Kinesis-source); Job 3 e Job 5 escrevem diretamente no Firehose (**Direct Put**, sem Kinesis intermediário).
...
- **Job 3 (Block Data Crawler)**: Busca dados completos de blocos e distribui hashes de transações em SQS. Publica dados do bloco diretamente no **Firehose Direct Put** (`mainnet-blocks-data`).
- **Job 4 (Mined Txs Crawler)**: Consome SQS, busca dados brutos de transações com **6 réplicas** usando rotação distribuída de API keys via semáforo DynamoDB. Produz em Kinesis (`mainnet-transactions-data`).
- **Job 5 (Txs Input Decoder)**: Consome Kinesis (`mainnet-transactions-data`), decodifica o campo `input` de transações usando ABIs do Etherscan, 4byte.directory e cache DynamoDB. Publica resultado diretamente no **Firehose Direct Put** (`mainnet-transactions-decoded`).
...
| **Firehose Direct Put** | `firehose-mainnet-blocks-data-{env}` | Dados completos de blocos (Job 3 → Firehose → S3 `raw/mainnet-blocks-data/`) |
| **SQS** | `mainnet-block-txs-hash-id-{env}` | Coordenação Job 3 → Job 4 (hashes de transações) |
| **Kinesis** | `mainnet-transactions-data-{env}` | Dados brutos de transações (PROVISIONED 1 shard) → Firehose → S3 (`raw/mainnet-transactions-data/`) |
| **Firehose Direct Put** | `firehose-mainnet-transactions-decoded-{env}` | Transações com input decodificado (Job 5 → Firehose → S3 `raw/mainnet-transactions-decoded/`) |

### 3. Firehose (S3 Delivery)

Firehose entrega arquivos NDJSON no S3, particionados por hora. Existem dois modos de entrega:
- **Kinesis-source**: consome de um Kinesis Data Stream (`mainnet-transactions-data`)
- **Direct Put**: aplicação escreve diretamente no Firehose sem Kinesis intermediário (`mainnet-blocks-data`, `mainnet-transactions-decoded`)
- **CloudWatch Logs source**: logs de aplicação entregues via assinatura Firehose
...
|------------|-------------|------------|
| **Kinesis Data Streams** | AWS Kinesis (PROVISIONED 1 shard, `-dev` suffix) | AWS Kinesis (PROVISIONED 1 shard, `-prd` suffix) |
| **Firehose Direct Put** | Firehose `mainnet-blocks-data-dev`, `mainnet-transactions-decoded-dev` | Firehose `mainnet-blocks-data-prd`, `mainnet-transactions-decoded-prd` |
...
| **S3 Storage** | Bucket `dm-chain-explorer-dev-ingestion` | Bucket `dm-chain-explorer-lakehouse` (raw + exports) + bucket databricks |
...
│  ├─ Kinesis Data Streams (1 stream PROVISIONED — mainnet-txs-data) │
│  ├─ Firehose Direct Put (2 streams — blocks, decoded-txs)          │
│  ├─ Kinesis Firehose (4 delivery streams total → S3)              │
│  ├─ SQS (2 filas + 2 DLQs)                                      │
