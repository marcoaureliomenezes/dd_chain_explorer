...
Acionado manualmente. Executa ambos os pipelines DLT com `full_refresh: true`, descartando checkpoints e reprocessando todos os dados disponíveis na fonte. Ao final exporta as tabelas Gold para S3:
```
full_refresh_ethereum → full_refresh_app_logs → export_gold_to_s3
```
...
Os schedules dos pipelines DLT são configurados diretamente nos arquivos YAML (chave `schedule`), sem necessidade de um workflow separado de trigger:
...
> **Nota**: Os pipelines são deployados com `pause_status: PAUSED`. Em PROD, os pipelines são automaticamente despausados pelo target `prod` no `databricks.yml`.
...
| `dm-dlt-full-refresh` | Reprocessamento completo DLT (full_refresh) + export Gold → S3 |
...
| Workflow | Descrição |
|----------|----------|
| `dm-trigger-dlt-ethereum` | Dispara `dm-ethereum` uma vez (full_refresh=false) |
| `dm-trigger-dlt-app-logs` | Dispara `dm-app-logs` uma vez (full_refresh=false) |
