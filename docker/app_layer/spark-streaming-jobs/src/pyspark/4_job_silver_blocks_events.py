...
class SilverBlocksEvents(IDmStreaming):
  ...
  def config_sink( self, tables_output: Dict[str, str],  sink_properties: Dict[str, str]):
    self.silver_mined_blocks = tables_output["silver_mined_blocks"]
    ...
    self.logger.info(f"Sink configured with tables {self.silver_mined_blocks}")
  ...
  def transform(self):
    self.df_streaming = (
      self.df_streaming
        ...
        .select("ingestion_time", "key", "data.*")
        .withColumn("block_type", col("key").cast(StringType()))
        .withColumn("block_number", col("block_number").cast(LongType()))
        .withColumn("block_timestamp", to_timestamp(col("block_timestamp")))
  ...
  def load(self) -> None:
    return (
      self.df_streaming
        ...
        .toTable(self.silver_mined_blocks)
        .awaitTermination())
...
if __name__ == "__main__":
  ...
  engine = (
    SilverBlocksEvents(LOGGER, spark)
