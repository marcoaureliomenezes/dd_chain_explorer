...
|--------|------------|----------|
| `services/dev/01_peripherals/` | `main.tf` | S3, DynamoDB, Kinesis (1 stream PROVISIONED + Firehose), SQS + DLQs, CloudWatch Logs + Firehose |
| `services/dev/02_lambda/` | `main.tf` | Lambda `dm-chain-explorer-dev-gold-to-dynamodb` (S3 event → DynamoDB sync) |

> **HML**: infra parcialmente gerenciada por Terraform em `services/hml/` (IAM, Firehose, periféricos, ECS). Os recursos efêmeros (ECS cluster, SG) são criados/destruídos no pipeline CI/CD (`deploy_all_dm_applications.yml`).
...
| `04_peripherals/` | Kinesis (1 stream PROVISIONED + 2 Firehose Direct Put) + Firehose (4 streams) + SQS + DLQs + CloudWatch + S3 (lakehouse + databricks) + DynamoDB | **Pago** |
...
| `deploy_all_dm_applications.yml` | `workflow_dispatch` (develop) | Streaming apps + DABs + Lambda em pipeline único |
...
### 3.3 Deploy All DM Applications (`deploy_all_dm_applications.yml`)

Workflow unificado que deploya os três tipos de aplicação em um único pipeline coordenado.

```mermaid
flowchart TD
    BG["branch-guard"] --> CV["all-check-version<br/>(3 tag suffixes: v, -dabs, -lambda)"]
    CV --> SB["stream-build-rc"] & DV["dabs-validate"] & LB["lambda-build"]
    SB & DV & LB --> HP["hml-provision<br/>(efêmero)"]
    HP --> HC["hml-deploy-ecs"]
    HC --> HD["hml-deploy-dabs"]
    HD --> HL["hml-deploy-lambda"]
    HL --> HT["hml-test: streaming + dabs + lambda"]
    HT --> HW["hml-teardown"]
    HT --> PI["check-prd-infra"]
    PI --> PS["prd-deploy-streaming"]
    PS --> PD["prd-deploy-dabs"]
    PD --> PL["prd-deploy-lambda"]
    PL --> PT["prd-create-tags<br/>(v{VER}, v{VER}-dabs, v{VER}-lambda)"]
```

**Fases:**
1. **CHECK** — valida versão (3 tags não existem)
2. **BUILD** — Docker RC + Lambda artifacts + DABs validate (paralelo)
3. **HML DEPLOY** — provisiona HML efêmero → ECS → DABs → Lambda (sequencial)
4. **HML TEST** — streaming + DLT + Lambda
5. **HML TEARDOWN** — sempre executado (mesmo em falha)
6. **CHECK PRD INFRA** — só se HML passou
7. **PRD DEPLOY** — Streaming → DABs → Lambda → tags
...
|----------|-------------------------|-------------------|
| `deploy_all_dm_applications` (streaming) | `02_vpc`, `03_iam`, `04_peripherals`, `07_ecs` | TF remote state + GitHub Secrets (HML VPC/subnet) |
| `deploy_all_dm_applications` (dabs) | `05_databricks` | GitHub Secrets (OAuth creds, workspace URL) |
| `deploy_all_dm_applications` (lambda) | `03_iam`, `04_peripherals`, `06_lambda` | TF remote state (S3) |
...
| CI/CD Aplicações | `.github/workflows/deploy_all_dm_applications.yml` |
